...
function open(port, openUrl) {
    if (openUrl) {
        ...
        openUrl = `--app=http://127.0.0.1:3000/`
    }
    ...
    global.HOSTRC_COMMAND = command

    if (process.env.HOSTRC_OPEN === 'false'){
       console.log("current open = false, if open please > export HOSTRC_OPEN=true")
        return
    }
